...
export const ProjectAddOnCalculator = ({
  ...
  const formatDualCurrency = (amount: number) => {
    const usd = amount < 10000 ? amount : Math.round(amount / 280);
    const pkr = amount < 10000 ? amount * 280 : amount;
    return `$${usd.toLocaleString()} (PKR ${pkr.toLocaleString()})`;
  };

  const formattedAddOnNames = selectedPackages.length
    ? selectedPackages.map((p) => `${p.title} (+${formatDualCurrency(p.price)})`).join(", ")
  ...
  const whatsappMessage = encodeURIComponent(
    `Hi Vemzo Technologies! I am placing an order for:\n` +
      `📦 Package: ${projectTitle} (${formatDualCurrency(basePrice)})\n` +
      `➕ Selected Add-Ons: ${formattedAddOnNames}\n` +
      `💰 Estimated Total Price: ${formatDualCurrency(finalTotal)}${formattedCustomNote}\n\n` +
    ...
    <div className="rounded-3xl border border-[rgba(0,220,130,0.3)] bg-[radial-gradient(ellipse_at_top,rgba(0,220,130,0.08),transparent_80%)] p-6 sm:p-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 pb-6 border-b border-gray-800">
        ...
        <div className="bg-[#09090b] border border-[rgba(0,220,130,0.4)] rounded-2xl p-4 text-right shrink-0">
          ...
          <div className="text-xl sm:text-2xl font-black text-[#00dc82] mt-0.5">{formatDualCurrency(finalTotal)}</div>
          <div className="text-[11px] text-gray-500">Base {formatDualCurrency(basePrice)} + {formatDualCurrency(addOnsTotal)} Add-Ons</div>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between pt-3 border-t border-gray-800/60 text-xs">
                ...
                <span className={`font-bold text-xs ${isSelected ? "text-[#00dc82]" : "text-white"}`}>
                  +{formatDualCurrency(pkg.price)}
                </span>
              </div>
            </div>
          );
